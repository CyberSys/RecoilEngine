...
void CProjectileDrawer::Init() {
	...
	fxShaders[0] = shaderHandler->CreateProgramObject("[ProjectileDrawer::VFS]", "FX Shader (wo dist)");
	fxShaders[1] = shaderHandler->CreateProgramObject("[ProjectileDrawer::VFS]", "FX Shader (w  dist)");
	for (size_t i = 0; i < 2; ++i) {
		auto* fxShader = fxShaders[i];
		...
		fxShader->SetFlag("SMOOTH_PARTICLES", true);
		fxShader->SetFlag("WITH_DISTORTION", i == 1);
	...
	fxCombShader->SetUniform("params", 0.0f, 0.0f);
...
void CProjectileDrawer::DrawAlpha(bool drawAboveWater, bool drawBelowWater, bool drawReflection, bool drawRefraction)
{
	...
	static uint32_t prevDrawFrame = uint32_t(-1);
	static uint8_t prevPassMask = 0U;
	const uint8_t thisPassMask =
	...
	// spare some compute on drawAboveWater, drawBelowWater passes (the result is the same)
	if (globalRendering->drawFrame != prevDrawFrame || thisPassMask != prevPassMask) {
		prevDrawFrame = globalRendering->drawFrame;
		prevPassMask = thisPassMask;
...

	{
		ZoneScopedN("ProjectileDrawer::DrawAlpha(RR)");
		{
			...
			const bool needSoften = (wantSoften > 0) && mainPass;
			if (needSoften) {
			...
			auto* fxShader = fxShaders[mainPass];
			fxShader->Enable();
...
{
	{
		...
		distortionTex = GL::Texture2D(globalRendering->viewSizeX, globalRendering->viewSizeY, GL_RGB16F/*GL_RG16_SNORM*/, tcp, false);
	}
	{
		static constexpr int32_t NUM_MIPS = 4;
		GL::TextureCreationParams tcp{
			//make function re-entrant
			.texID = screenCopyTex.GetId(),
			.reqNumLevels = NUM_MIPS,
